        ...
        let sum = entries
            .map(|(i, o)| {
                ...
                digits
                    ...
                    .into_iter()
                    .enumerate()
                    .map(|(i, d)| (d as u32) * (10 as u32).pow((digits.len() - i - 1) as u32))
...
fn map_digits(input: Vec<String>) -> HashMap<String, u8> {
    ...
    for combination in input.clone().into_iter() {
        match combination.len() {
            2 => {
                map.insert(1, combination);
            }
            3 => {
                map.insert(7, combination);
            }
            4 => {
                map.insert(4, combination);
            }
            5 => {
                two_three_or_five.push(combination);
            }
            6 => {
                zero_six_or_nine.push(combination);
            }
            7 => {
                map.insert(8, combination);
            }
            _ => {}
        }
    }
    ...
    zero_six_or_nine.into_iter().for_each(|v| {
        let is_zero_or_nine =
            map.get(&1)
                .unwrap()
                .clone()
